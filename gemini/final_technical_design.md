...
*   **Vanilla JavaScript:** Used for DOM manipulation and simple event handling.
...
*   **Complex State Management:** Managing complex UI states (e.g., multi-step forms, dynamic filtering, shopping carts) with traditional imperative JavaScript approaches is error-prone and leads to "spaghetti code."
...
| Interactivity | Low (Traditional JS) | High (React) | **High (React Islands)** |
...
**Authoring Mode Logic Flow:**

```mermaid
stateDiagram-v2
    [*] --> CheckWCMMode
    
    state CheckWCMMode <<choice>>
    
    CheckWCMMode --> EditMode: Cookie/Data attr = "edit"
    CheckWCMMode --> PreviewMode: Cookie/Data attr = "preview"
    CheckWCMMode --> DisabledMode: No attr (Published)
    
    state EditMode {
        [*] --> RenderPlaceholder
        RenderPlaceholder --> ShowConfigSummary
        ShowConfigSummary --> [*]
    }
    
    state PreviewMode {
        [*] --> LoadReactChunk
        LoadReactChunk --> HydrateApp
        HydrateApp --> InteractiveState
    }
...
sequenceDiagram
    ...
    participant HTML as AEM Page (HTML)
    participant Script as Widget Engine (main.js)
    participant Network as Network
    participant React as React Runtime

    Browser->>HTML: Parse Document
    Note over HTML: Encounter <div data-widget="Calculator">
    Browser->>Script: Load & Execute Clientlib
    Script->>HTML: DOMContentLoaded Event
    Script->>Script: initWidgets() (Scan DOM)
    
    loop For Each Widget Found
        Script->>Script: Check WCM Mode
        alt Edit Mode
            Script->>HTML: Render Static Placeholder
        else Preview/Publish Mode
            Script->>Script: Check Registry
            Script->>Network: Fetch Component Chunk (Lazy)
            Network-->>Script: Return JS Chunk
            Script->>React: createRoot() & render()
            React->>HTML: Hydrate Widget UI
        end
    end
```
...
function mountWidget(container) {
  ...
  // 3. React 19 Mounting with Suspense (required for lazy loading)
...
// When a component is edited/re-mounted in the AEM editor, we must clean up and re-mount.
// Note: `$(document).on(...)` is part of AEM's legacy event handling mechanism,
// typically used for AEM's editor interface. This specific AEM hook does not
// introduce a separate dependency for the React application's runtime.
...
### 6.5 React 19 Considerations

This architecture is designed to be fully compatible with React 19. Key considerations for the upgrade include:

*   **React Compiler:** The build pipeline should be configured to use `babel-plugin-react-compiler` to automatically optimize re-renders, reducing the need for manual `useMemo` and `useCallback`.
*   **New Hooks:** Developers are encouraged to use the new `use()` hook for data fetching within components, simplifying the async logic compared to `useEffect`.
*   **SEO Metadata:** **Constraint:** React 19's native support for document metadata (title, meta tags) must **NOT** be used. AEM/Sling retains exclusive control over the document `<head>` to ensure correct SEO and metadata management.
*   **Deprecated Features:** Avoid `defaultProps` for functional components; use ES6 default parameters instead.
...
flowchart LR
    ...
    subgraph "Vite Build Process"
        A & B --> C[Vite / Rollup]
        C -->|Tree Shaking| D[Optimized Bundles]
        C -->|Code Splitting| E[Chunk Hashing]
    end

    subgraph "Output / Dist"
        D & E --> F[dist/js/main.js]
        D & E --> G[dist/js/chunks/*.js]
        D & E --> H[manifest.json]
    end

    subgraph "AEM Integration"
        F & G & H --> I[aem-clientlib-generator]
        I --> J[ui.apps/src/.../clientlibs]
    end
...
*A property is a characteristic or behavior that should hold true across all valid executions of a system. These properties guide property-based testing.*
